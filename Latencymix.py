...
# Replace Latency >200 with the mean value
...
# Replace Latency >200 with the mean from extended training
...
lags_train = pd.concat(
    [make_lags(df_train_extended[feat], lags=5, prefix=feat) for feat in input_features],
    axis=1
)
Latency_target_train = make_multistep_target(df_train_extended["Latency"], steps=2, prefix="Latency")
...
lags_val = pd.concat(
    [make_lags(df_val_new[feat], lags=5, prefix=feat) for feat in input_features],
    axis=1
)
Latency_target_val = make_multistep_target(df_val_new["Latency"], steps=2, prefix="Latency")
...
# 10. Inverse Scale Latency Predictions 
...
writer.add_figure('Validation Latency Steps_mix', fig, global_step=0)
writer.close()
